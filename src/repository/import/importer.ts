...
export default class Importer {
    ...
    constructor(public repository: Repository, public text: string) {
        ...
        if (XML.isValid(xmlDoc) && xmlDoc.documentElement.tagName == 'definitions') {
            this.parseDocuments(xmlDoc);
        }
    }

    async parseDocuments(xmlDoc: Document) {
        ...
        XML.getChildrenByTagName(xmlDoc.documentElement, 'caseFileItemDefinition').forEach(async xmlElement => {
            const fileName = getAttribute(xmlElement, 'id');
            if (isNew(fileName)) {
        ...
        XML.getElementsByTagName(xmlDoc.documentElement, 'caseFileModel').forEach(async caseFileModel => {
