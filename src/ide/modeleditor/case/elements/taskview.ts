...
export default abstract class TaskView<TD extends TaskDefinition = TaskDefinition> extends TaskStageView<TD> {
    ...
    async generateNewTaskImplementation() {
        ...
        if (existingModel) {
            ...
            const fileName = await ModelEditorMetadata.createNewModel(this.canvas.editor.ide, this.fileType, potentialImplementationName, this.definition.documentation.text);
    ...
    get markup() {
        return `<g @selector="scalable">
                    <rect @selector='body' rx="5" ry="5" width="100" height="60" ></rect>
                </g>
                <text @selector="label" ></text>
                <image x="0" y="-4" width="24" height="24" href="${this.imageURL}" ></image>
